...
class Daftar_lama extends CI_Controller
{
    ...
    public function cari()
    {
        ...
        $data['santri'] = $this->DaftarModel->cariNis($nis);
    ...
    public function daftarActOkJKkjhkjdhfupohi()
    {
        ...
        $cekNIS2 = $this->DaftarModel->cekNIS2($nis)->num_rows();
        $cekPa = $this->DaftarModel->cekPa($nik)->num_rows();
        $cekPi = $this->DaftarModel->cekPi($nik)->num_rows();
        ...
        if ($jl <= $g1) {
            $gel = "1";
            $by = 'Rp. 25.000 (50% dari biaya pendaftaran santri baru)';
        } else if ($jl > $g1 && $jl <= $g2) {
            $gel = "2";
            $by = 'Rp. 50.000 (50% dari biaya pendaftaran santri baru)';
        } else if ($jl >= $g3) {
            $gel = "3";
            $by = 'Rp. 75.000 (50% dari biaya pendaftaran santri baru)';
        }
...
Data yang anda isi telah  tersimpan di data panitia Penerimaan santri lanjutan PP. Darul Lughah Wal Karomah, atas :
...
_*NB : Calon Santri diwajibkan memakai baju putih songkok/kerudung hitam & Bawahan bebas (Ketika tes dan berangkat mondok)*_';
        ...
        $pesan2 = '*Info tambahan santri lanjutan*
...
*Terimakasih*';
        ...
        if ($cekNIS2 >= 1) {
            echo "
            <script>
                alert('Maaf. NIS Sudah ada');
                window.location = '" . base_url('daftar_lama') . "';
            </script>
            ";
        } else {
            ...
            if ($cekNik == 1) {
                ...
                if ($cekPa == 50) {
                    echo "
                    <script>
                        alert('Maaf. Untuk SMP Putra sudah penuh');
                        window.location = '" . base_url('daftar_lama') . "';
                    </script>
                    ";
                } else {
                    if ($cekPi == 50) {
                        echo "
                        <script>
                            alert('Maaf. Untuk SMP Putri sudah penuh');
                            window.location = '" . base_url('daftar_lama') . "';
                        </script>
                    ";
                    } else {
